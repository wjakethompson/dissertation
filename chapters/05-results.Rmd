...
Critical to the model selection process is model fit. Although different model reduction processes may all show adequate recovery of item parameters and respondent classification, this measures cannot provide insight as to whether or not the removal of the parameters significantly impacted model fit. Figure \@ref(fig:pvalue-aic), Figure \@ref(fig:pvalue-bic), and Figure \@ref(fig:pvalue-abic) show how many times the AIC, BIC, and aBIC, respectively, selected each model reduction as the preferred model for a given data set.
...
p <- fit_results %>%
  ...
  geom_col(width = 0.7, position = position_dodge(width = 0.7)) +
  scale_fill_OkabeIto() +
  labs(x = "Sample Size", y = "aBIC Selections", fill = "Reduction\nProcess") +
  theme_bw() +
  theme(legend.position = "bottom", panel.grid.major.x = element_blank())
