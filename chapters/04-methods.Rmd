...
Finally model fit will be evaluated using the aggregated univariate and bivariate $\chi^2$ statistics [@rupp_diagnostic_2010]. These statistics are summed from item or item pair level $\chi^2$ statistics that compare the observed and expected counts of the response patterns for the item or pair of items. These statistics are larger when there is a greater amount of misfit, however, because the individual item-level indices are not independent of each other, the aggregated statistic doesn't follow the true asymptotic $\chi^2$ distribution. Thus, only the sum is used to assess fit, not the associated p-value. The evaluation of model fit will allow for an investigation into whether certain model reduction processes result in models that fit worse than the saturated model.

### Software

As with the pilot study, all model were estimated using *Mplus* version 7.4 [@muthen_mplus_1998] via the **MplusAutomation** package [@R-MplusAutomation] in *R* version `r getRversion()` [@R-base]. *Mplus* code for the estimation of the LCDM was generated in *R* using custom scripts based on the work of @rupp_files_2012 and @templin_obtaining_2013. Data generation and evaluation of the models was carried out in *R* using the **dplyr** [@R-dplyr], **forcats** [@R-forcats], **lubridate** [@R-lubridate], **portableParallelSeeds** [@R-portableParallelSeeds], **purrr** [@R-purrr], **readr** [@R-readr], **stringr** [@R-stringr], and **tidyr** [@R-tidyr], **tidyselect** [@R-tidyselect] packages.
